...
This workflow extracts gRNAs from the coding region of 7 genes in S. cerevisiae genome and retrieves promiscuous gRNAs with no mismatches. No parallelization is called (chunk = 1, nchunks = 1). The top 10 promiscuous gRNAs are returned.
...
param.df  <- data.frame(Parameter=names(unlist(params)),Value=unlist(params),Description=ifelse("selgenes"%in%names(params),param.desc,param.desc[-10]))
...
if (!exists("hitsindexes")){
  ...
  if (!any(grepl("hitlist",dir(interimdir)) & 
           grepl(paste0("nmismatch_",nmismatches),dir(interimdir)) &
           grepl(region,dir(interimdir)))){
    message("No hitlist for region with selected mismatches found in directory: ", interimdir,". Skipping...")
  } else {
    hitsindexes <- lumphitsindexes(interimdir,region,nmismatches)
  }
}
...
analyzealignments(hitsindexes,
                  ...
                  filename   =paste0("res/top",toprank,"gRNA_hitlistin",prefix,"_nmismatches",nmismatches,
                       ifelse(!is.null(tokeepgenes),"_annotatedonly.csv",".csv"))
                  )
```
